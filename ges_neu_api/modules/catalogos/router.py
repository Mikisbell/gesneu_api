...
def get_catalog_service(db: AsyncSession = Depends(get_session)) -> CatalogService:
    """Dependency para obtener el servicio de catálogos."""
    return CatalogService(db)


# NOTA: Los endpoints para fabricantes se encuentran en el módulo de neumáticos
...
async def create_proveedor(
    ...
    return await service.create_proveedor(proveedor_data)
...
async def get_proveedores(
    ...
    return await service.get_proveedores(skip, limit)
...
async def get_proveedor(
    ...
    proveedor = await service.get_proveedor(proveedor_id)
    if not proveedor:
...
async def update_proveedor(
    ...
    proveedor = await service.update_proveedor(proveedor_id, proveedor_data)
    if not proveedor:
        raise HTTPException(status_code=404, detail="Proveedor no encontrado")
    return proveedor
...
async def delete_proveedor(
    ...
    success = await service.delete_proveedor(proveedor_id)
    if not success:
        raise HTTPException(status_code=404, detail="Proveedor no encontrado")
...
async def create_motivo_desecho(
    ...
    return await service.create_motivo_desecho(motivo_data)
...
async def get_motivos_desecho(
    ...
    return await service.get_motivos_desecho(skip, limit)
...
async def get_motivo_desecho(
    ...
    motivo = await service.get_motivo_desecho(motivo_id)
    if not motivo:
...
async def update_motivo_desecho(
    ...
    motivo = await service.update_motivo_desecho(motivo_id, motivo_data)
    if not motivo:
        raise HTTPException(status_code=404, detail="Motivo de desecho no encontrado")
    return motivo
...
async def delete_motivo_desecho(
    ...
    success = await service.delete_motivo_desecho(motivo_id)
    if not success:
        raise HTTPException(status_code=404, detail="Motivo de desecho no encontrado")
...
async def create_almacen(
    ...
    return await service.create_almacen(almacen_data)
...
async def get_almacenes(
    ...
    return await service.get_almacenes(skip, limit)
...
async def get_almacen(
    ...
    almacen = await service.get_almacen(almacen_id)
    if not almacen:
...
async def update_almacen(
    ...
    almacen = await service.update_almacen(almacen_id, almacen_data)
    if not almacen:
        raise HTTPException(status_code=404, detail="Almacén no encontrado")
    return almacen
...
async def delete_almacen(
    ...
    success = await service.delete_almacen(almacen_id)
    if not success:
        raise HTTPException(status_code=404, detail="Almacén no encontrado")
...
async def create_parametro_inventario(
    ...
    return await service.create_parametro_inventario(parametro_data)
...
async def get_parametros_inventario(
    ...
    return await service.get_parametros_inventario(skip, limit)
...
async def update_parametro_inventario(
    ...
    parametro = await service.update_parametro_inventario(parametro_id, parametro_data)
    if not parametro:
        raise HTTPException(status_code=404, detail="Parámetro de inventario no encontrado")
    return parametro
...
async def delete_parametro_inventario(
    ...
    success = await service.delete_parametro_inventario(parametro_id)
    if not success:
        raise HTTPException(status_code=404, detail="Parámetro de inventario no encontrado")
