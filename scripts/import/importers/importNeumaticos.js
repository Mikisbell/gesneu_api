...
class NeumaticosImporter {
    ...
    async import() {
        this.logger.startPhase('Histórico de Neumáticos');

        try {
            // 1. Cargar caché de vehículos y modelos
            await this.loadCache();

            const sheets = [
                config.SHEETS.NEUMATICOS_TRACTOS,
                config.SHEETS.NEUMATICOS_VOLQUETE,
                config.SHEETS.NEUMATICOS_LINEA_AMARILLA
            ];

            for (const sheetName of sheets) {
                this.logger.info(`Procesando hoja: ${sheetName}`);
                await this.importSheet(sheetName);
            }
        } catch (error) {
            this.logger.error('Error en importación de neumáticos', error);
            throw error;
        }
    }
    ...
    async loadCache() {
        try {
            const vehiculos = await prisma.vehiculo.findMany({
                select: { id: true, codigo_interno: true, placa: true }
            });

            vehiculos.forEach(v => {
                if (v.codigo_interno) this.vehiculosMap.set(v.codigo_interno, v.id);
                if (v.placa) this.vehiculosMap.set(v.placa, v.id);
            });

            const modelos = await prisma.modeloNeumatico.findMany();
            modelos.forEach(m => this.modelosMap.set(m.nombre.toUpperCase(), m.id));

            this.defaultModelo = await prisma.modeloNeumatico.findFirst({
                where: { nombre: 'GENERICO' }
            });
        } catch (error) {
            this.logger.error('Error cargando caché', error);
            throw error;
        }
    }
    ...
    async importSheet(sheetName) {
        try {
            ...
            if (!worksheet) {
                this.logger.warn(`Hoja ${sheetName} no encontrada`);
                return;
            }
