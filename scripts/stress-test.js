...
async function authenticate() {
    ...
    // 1. Get CSRF Token
    // Dynamic import for fetch if needed (Node 18+ has fetch global)
...
function updateCookieJar(jar, setCookieHeader) {
...
function runAutocannon(url, cookie, name) {
    console.log(`\n🚀 Starting Stress Test: ${name}`);
    console.log(`   Target: ${url}`);

    // -c 10 connections, -d 5 seconds, -j json output
...
async function main() {
    try {
        ...
        console.table(results.map((r, i) => ({
            ...
            RPS: r?.requests.average ? r.requests.average.toFixed(2) : 'N/A',
            'Latency(ms)': r?.latency.average ? r.latency.average.toFixed(2) : 'N/A',
            Errors: r?.errors || 0
        })));
